...
cat > /root/README <<'DOC'
================================================================
  ПЕСОЧНИЦА OKVION — настоящий Linux (busybox) в браузере
================================================================

Это НЕ имитация: команды выполняются по-настоящему. Пробуй что
угодно — кнопка «Перезапустить» вернёт чистую систему с нуля.

Быстрые команды:
  help    показать эту справку снова
  tasks   показать список заданий

----------------------------------------------------------------
УЧЕБНЫЙ СЕРВИС  (/root/app) — имитация работающего приложения
----------------------------------------------------------------
  sh /root/app/start.sh     запустить сервис (начнёт писать лог)
  sh /root/app/status.sh    статус + последние строки лога
  tail -f /root/app/app.log смотреть лог вживую  (Ctrl+C — выйти)
  sh /root/app/stop.sh      остановить сервис

Файлы проекта:
  /root/app/service.sh   сам «сервис»: цикл, который пишет в app.log
  /root/app/app.log      лог сервиса (появится после запуска)
  /root/app/access.log   пример лога веб-сервера — для анализа

----------------------------------------------------------------
АНАЛИЗ ЛОГОВ  (тренируйся на /root/app/access.log)
----------------------------------------------------------------
  grep " 500 " /root/app/access.log        найти ошибки 500
  awk '{print $1}' /root/app/access.log | sort | uniq -c | sort -rn
                                           топ IP по числу запросов
  wc -l /root/app/access.log               сколько всего строк
  tail -n 20 /root/app/app.log             последние 20 строк лога

----------------------------------------------------------------
ПОЛЕЗНЫЕ КОМАНДЫ
----------------------------------------------------------------
  ls -la   cd   cat   head   tail   cp   mv   rm   mkdir
  grep   awk   sed   sort   uniq   wc   cut   find
  vi ФАЙЛ   (i — ввод, Esc, :wq — сохранить, :q! — выйти)
  ps   top (q — выход)   kill   df -h   free   uname -a   ip a

----------------------------------------------------------------
ЗАДАНИЯ
----------------------------------------------------------------
Полный список — команда  tasks  (или файл /root/TASKS.txt).
Выполняй по очереди и жми «Проверить» справа — платформа сама
зайдёт в Linux и засчитает выполненные.

----------------------------------------------------------------
КНОПКИ СПРАВА / СВЕРХУ
----------------------------------------------------------------
  Проверить      проверить задания
  Проект         заново разложить учебный проект и этот README
  Файл           загрузить свой файл/скрипт в /root
  Скачать        забрать файл из песочницы к себе
  Перезапустить  чистая система с нуля
================================================================
DOC
alias help='cat /root/README'
alias tasks='cat /root/TASKS.txt'
clear
echo '================================================================'
echo '  Песочница Okvion готова.  Учебный проект: /root/app'
echo ''
echo '  help   — краткая справка и все команды'
echo '  tasks  — список заданий (выполняй и жми «Проверить»)'
echo '================================================================'
`;
...
// Файл /root/TASKS.txt со списком заданий — генерируем из TASKS, чтобы он
// всегда совпадал с тем, что показано в интерфейсе. Пишем через heredoc с
// «закавыченным» разделителем, поэтому текст заданий берётся как есть.
function buildTasksFile(): string {
  const body = TASKS.map((t, i) => `${i + 1}. ${t.text}`).join("\n");
  return (
    `cat > /root/TASKS.txt <<'OKVTASKS'\n` +
    `ЗАДАНИЯ ПЕСОЧНИЦЫ OKVION\n` +
    `Выполняй по очереди и жми «Проверить» справа.\n\n` +
    `${body}\n` +
    `OKVTASKS\n`
  );
}
...
export default function SandboxPage() {
  ...
  const injectProject = useCallback(() => {
    ...
    // Сначала файл заданий (из актуального списка), затем разбор проекта.
    send(buildTasksFile() + SETUP_SCRIPT.trim() + "\n");
