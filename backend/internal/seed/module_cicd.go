		...
		Lessons: []LessonSeed{
			{
				...
				Content: map[string]any{
					"body": "## 🎯 Чему вы научитесь\n\n" +
						"- Объяснить разницу между CI и CD и почему дешёвые проверки идут первыми\n" +
						"- Настроить workflow GitHub Actions: запуск, тесты, сборка образа, секреты вместо паролей\n" +
						"- Найти упавший шаг пайплайна через gh и разобрать вывод сборки образа\n" +
						"- Выбрать стратегию выката (rolling, blue-green, canary) и откатить через git revert\n" +
						"- Объяснить GitOps: Git как источник правды, выкат равен коммиту, откат равен revert\n" +
						"\n" +
						"## 📋 Что нужно знать заранее\n\n" +
						"- Работать в терминале Linux: базовые команды и пути к файлам (глава 2)\n" +
						"- Git: коммиты, ветки, pull request и слияние в main (глава 3)\n" +
						"- Docker: собирать образ, понимать теги версий и реестр образов (глава 5)\n" +
						"\n" +
						"## Зачем это нужно\n\n" +
						...
						"    steps:\n" +
						"      - uses: actions/checkout@v5   # забрать код\n" +
						...
						"## ❓ Проверьте себя\n\n" +
						"Почему быстрые тесты в конвейере ставят ПЕРЕД сборкой образа, а не после неё?\n\n" +
						"*Ответ: дешёвое и быстрое — раньше. Если тест упал, нет смысла тратить минуты на сборку.*\n\n" +
						"## Запомнить\n\n" +
			...
			},
			{
				...
				Content: map[string]any{
					"body": "## Зачем это нужно\n\n" +
						...
						"> Дальше встретятся слова «кластер», «манифест», «Deployment», `kubectl` — их подробно " +
						"разберём в главе про Kubernetes. Сейчас достаточно одной идеи: агент приводит прод " +
						...
						"## ❓ Проверьте себя\n\n" +
						"Инженер через `kubectl` откатил образ на старую версию, но через пару минут кластер снова показал новую. Почему так вышло?\n\n" +
						"*Ответ: агент GitOps постоянно сверяет кластер с описанием в Git и возвращает то, что записано там. Ручная правка мимо Git не сохранилась, поэтому агент её отменил. Так прод защищён от тихих изменений в обход репозитория.*\n\n" +
						"## Запомнить\n\n" +
				...
				Content: map[string]any{
					...
					"questions": []map[string]any{
						...
						},
						{
							"id":   "q6z",
							"type": "order",
							"text": "Расставьте шаги конвейера доставки по порядку:",
							"items": []map[string]any{
								{"id": "s1", "text": "Разработчик отправляет коммит"},
								{"id": "s2", "text": "CI прогоняет тесты"},
								{"id": "s3", "text": "Собирается образ"},
								{"id": "s4", "text": "CD выкатывает на прод"},
								{"id": "s5", "text": "Мониторинг следит за результатом"},
							},
							"explanation": "Проверка до сборки, сборка до выката, наблюдение после — это и есть CI/CD.",
						},
						{
							"id":          "q7z",
							"type":        "blank",
							"text":        "Впишите двумя буквами практику автоматической проверки кода после каждого коммита.",
							"accept":      []string{"ci"},
							"hint":        "Continuous Integration.",
							"explanation": "CI — непрерывная интеграция: тесты и сборка запускаются на каждый коммит.",
						},
					},
				},
			},
