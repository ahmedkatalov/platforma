		...
		Lessons: []LessonSeed{
			{
				...
				Content: map[string]any{
					"body": "## 🎯 Чему вы научитесь\n\n" +
						"- Объяснить разницу между Terraform (создаёт) и Ansible (настраивает)\n" +
						"- Пройти цикл Terraform init - validate - plan - apply и читать план: add, change, destroy\n" +
						"- Написать конфиг Terraform с переменными, тегами и output, без паролей в коде\n" +
						"- Собрать идемпотентный плейбук Ansible и проверить его через --check и --limit\n" +
						"- Объяснить, зачем состояние Terraform хранят удалённо с блокировкой\n" +
						"\n" +
						"## 📋 Что нужно знать заранее\n\n" +
						"- Работать в терминале Linux: переходить по каталогам и запускать команды\n" +
						"- Понимать, что такое Git-репозиторий и что история коммитов не стирается\n" +
						"- Знать про доступ на сервер по SSH и что такое nginx\n" +
						"\n" +
						"## Зачем это нужно\n\n" +
						...
						"  ami           = \"ami-0abc123\"   # какой образ ОS брать\n" +
						...
						"## ❓ Проверьте себя\n\n" +
						"Вы руками поменяли настройку на сервере, созданном через Terraform. Что будет при следующем `apply`?\n\n" +
						"*Ответ: Terraform вернёт ресурс к состоянию из кода. Менять нужно описание, а не сервер руками.*\n\n" +
						"## Запомнить\n\n" +
			...
			},
			{
				...
				Content: map[string]any{
					"body": "## Зачем это нужно\n\n" +
						...
						"## ❓ Проверьте себя\n\n" +
						"Задача `shell: apt install nginx` при каждом повторном запуске снова помечается `changed`. Почему так и как починить?\n\n" +
						"*Ответ: `shell` просто выполняет команду каждый раз — идемпотентности нет. Возьмите модуль `apt` с `state: present`: он тронет сервер, только если nginx ещё не стоит.*\n\n" +
						"## Запомнить\n\n" +
				...
				Content: map[string]any{
					...
					"questions": []map[string]any{
						...
						},
						{
							"id":   "q7z",
							"type": "order",
							"text": "Расставьте шаги работы с Terraform по порядку:",
							"items": []map[string]any{
								{"id": "s1", "text": "Описать инфраструктуру в .tf"},
								{"id": "s2", "text": "terraform init"},
								{"id": "s3", "text": "terraform plan"},
								{"id": "s4", "text": "terraform apply"},
							},
							"explanation": "init готовит провайдеры, plan показывает будущие изменения, apply их применяет.",
						},
						{
							"id":          "q8z",
							"type":        "blank",
							"text":        "Впишите подкоманду Terraform, которая показывает план изменений перед применением: terraform …",
							"accept":      []string{"plan"},
							"hint":        "Показывает, что будет создано, изменено и удалено.",
							"explanation": "terraform plan заранее показывает изменения — особенно важна строка «to destroy».",
						},
					},
				},
			},
