...
func moduleExam() ModuleSeed {
	return ModuleSeed{
		...
		Lessons: []LessonSeed{
			...
			{
				Title:       "Капстоун-проект: сервис от кода до прода",
				Kind:        "text",
				Summary:     "Финальный проект: соберите все навыки курса в одном сервисе",
				DurationMin: 30,
				Content: map[string]any{
					"body": "## Зачем это нужно\n" +
						"\n" +
						"Экзамен проверяет знания, а капстоун — умение собрать их вместе. Именно готовый проект показывают на собеседовании: он доказывает, что вы умеете провести сервис весь путь, а не только отвечать на вопросы.\n" +
						"\n" +
						"Это не урок с автопроверкой, а самостоятельный проект. Ниже — задача, этапы и критерии, по которым вы сами проверите результат.\n" +
						"\n" +
						"## Задача\n" +
						"\n" +
						"Возьмите любой простой веб-сервис (свой или готовый пример) и проведите его от кода до работающего прода с мониторингом.\n" +
						"\n" +
						"## Этапы (чек-лист)\n" +
						"\n" +
						"1. **Git.** Репозиторий, ветки под задачи, осмысленные коммиты, `.gitignore` без секретов.\n" +
						"2. **Docker.** `Dockerfile` с фиксированной версией, не-root, многоэтапной сборкой; `docker-compose.yml` с базой.\n" +
						"3. **CI.** Конвейер GitHub Actions: тесты идут до сборки, затем сборка образа и пуш в реестр. Секреты — в секретах CI, не в коде.\n" +
						"4. **Выкат.** Деплой на сервер или манифесты Kubernetes: `Deployment` плюс `Service`, HTTPS через nginx или cert-manager.\n" +
						"5. **Мониторинг.** Эндпоинт метрик, хотя бы один осмысленный алерт, логи без секретов.\n" +
						"6. **Надёжность.** Бэкап базы по расписанию и проверка восстановления.\n" +
						"7. **Безопасность.** Секреты в хранилище, минимальные права, скан образа через Trivy.\n" +
						"\n" +
						"## Критерии готовности\n" +
						"\n" +
						"Проверьте себя честно — по пунктам, а не на глаз:\n" +
						"\n" +
						"- `git push` запускает конвейер, и упавший тест останавливает выкат.\n" +
						"- Сервис открывается по HTTPS; ошибки 500 и 502 вы понимаете и умеете диагностировать.\n" +
						"- Есть откат: `git revert` возвращает прошлую рабочую версию.\n" +
						"- Секретов нет в репозитории — проверьте историю через `git log` и `git show`.\n" +
						"- Бэкап делается автоматически, и вы **проверили восстановление**, а не только создание копии.\n" +
						"\n" +
						"## Как оформить результат\n" +
						"\n" +
						"- `README` с описанием, схемой и командами запуска.\n" +
						"- Публичная ссылка на репозиторий — прямо в резюме и портфолио.\n" +
						"\n" +
						"## ❓ Проверьте себя\n" +
						"\n" +
						"С чего разумно начать капстоун — с настройки Kubernetes или с Git-репозитория и Dockerfile?\n" +
						"\n" +
						"*Ответ: с Git и Dockerfile. Оркестрация нужна в конце, когда уже есть контейнер, который можно разворачивать.*\n" +
						"\n" +
						"## Запомнить\n" +
						"\n" +
						"1. Капстоун проходит весь путь: код, Git, контейнер, CI/CD, прод, мониторинг.\n" +
						"2. Готовность проверяют по чек-листу и критериям, а не на глаз.\n" +
						"3. Оформленный проект в портфолио — сильнейший аргумент на собеседовании.\n",
					"resources": []map[string]any{
						{"title": "roadmap.sh — DevOps Roadmap", "url": "https://roadmap.sh/devops", "note": "сверьтесь: какие навыки капстоун должен показать"},
						{"title": "GitHub Actions — документация", "url": "https://docs.github.com/actions", "note": "как собрать конвейер CI/CD для проекта"},
						{"title": "The Twelve-Factor App", "url": "https://12factor.net/ru/", "note": "принципы, по которым делают продакшн-сервисы"},
					},
				},
			},
		},
