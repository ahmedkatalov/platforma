		...
		Lessons: []LessonSeed{
			{
				...
				Content: map[string]any{
					"body": "## 🎯 Чему вы научитесь\n\n" +
						"- Запускать и останавливать контейнеры через docker run с флагами -d, -p, --name, ps, logs, stop\n" +
						"- Объяснить разницу между образом и контейнером и хранить данные в томах, а не внутри контейнера\n" +
						"- Собрать свой образ по Dockerfile и ускорить сборку порядком слоёв и многоэтапной сборкой\n" +
						"- Описать приложение с базой в docker-compose.yml и поднять всё командой docker compose up\n" +
						"- Найти причину падения контейнера через ps -a, logs, inspect (OOMKilled) и stats\n" +
						"\n" +
						"## 📋 Что нужно знать заранее\n\n" +
						"- Базовые команды терминала Linux и чтение логов вроде tail -f (глава 2)\n" +
						"- Что такое порт и как посмотреть занятые порты через ss (глава 4)\n" +
						"- Как работает HTTP-сервис за nginx и что значит ошибка 502 (глава 4)\n" +
						"\n" +
						"## Зачем это нужно\n\n" +
						...
						"Ядро — сердцевина операционной системы: управляет памятью, процессами и оборудованием. " +
						"Поэтому контейнер **лёгкий и быстрый** — свою ОС он с собой не тащит.\n\n" +
						...
						"## ❓ Проверьте себя\n\n" +
						"Контейнер запущен, `docker ps` его показывает, но снаружи сайт недоступен. Что забыли?\n\n" +
						"*Ответ: проброс порта — флаг `-p`, например `-p 8080:80`.*\n\n" +
						"## Запомнить\n\n" +
			...
			{
				Title:       "YAML за 10 минут: язык всех конфигов",
				Kind:        "text",
				Summary:     "Отступы, списки и пары ключ-значение — формат docker-compose, CI, Ansible и Kubernetes",
				DurationMin: 10,
				Content: map[string]any{
					"body": "## Зачем это нужно\n\n" +
						"Дальше в курсе почти все настройки пишут на YAML: docker-compose, GitHub Actions, " +
						"Ansible, Kubernetes, Prometheus. Синтаксис простой, но один сбитый отступ ломает весь файл.\n\n" +
						"Разберём YAML за десять минут — и дальше конфиги будут читаться легко.\n\n" +
						"## Что такое YAML\n\n" +
						"**YAML — формат для описания настроек, удобный человеку.** Файлы имеют расширение " +
						"`.yml` или `.yaml`. Внутри не программа, а просто аккуратно разложенные данные.\n\n" +
						"## Три кирпичика\n\n" +
						"**1. Пара «ключ: значение»** — двоеточие и пробел после него:\n" +
						"\n" +
						"```yaml\n" +
						"image: nginx\n" +
						"port: 8080\n" +
						...
						"```\n" +
						"\n" +
						"Здесь `host` и `port` находятся внутри `database`, потому что стоят с отступом.\n\n" +
						"**3. Список — через дефис:**\n" +
						"\n" +
						"```yaml\n" +
						"ports:\n" +
						"  - 80\n" +
						...
						"```\n" +
						"\n" +
						"## Главное правило: отступы только пробелами\n\n" +
						"YAML различает уровни **только по отступам и только пробелами**. " +
						"Табуляция запрещена — файл сломается. Обычно один уровень — это два пробела.\n\n" +
						"Настройте редактор так, чтобы клавиша Tab вставляла пробелы. Это снимает половину проблем.\n\n" +
						"## Как ломается один отступ\n\n" +
						"```yaml\n" +
						"database:\n" +
						"host: localhost      # нет отступа: host уже НЕ внутри database\n" +
						"```\n" +
						"\n" +
						"Из-за одного пропущенного отступа смысл меняется полностью, и программа настроится не так.\n\n" +
						"## Какие бывают значения\n\n" +
						"- строка: `name: api` (кавычки нужны редко);\n" +
						"- число: `replicas: 3`;\n" +
						"- да или нет: `enabled: true` либо `false`;\n" +
						"- пусто: `value:` (после двоеточия ничего).\n\n" +
						"## ❓ Проверьте себя\n\n" +
						"Почему YAML нельзя выравнивать клавишей Tab?\n\n" +
						"*Ответ: YAML понимает уровни только по пробелам. Таб он не считает отступом, и файл не разберётся.*\n\n" +
						"## Частые ошибки новичка\n\n" +
						"- **Табы вместо пробелов** — самая частая причина сломанного файла.\n" +
						"- **Разное число пробелов** на одном уровне.\n" +
						"- **Нет пробела после двоеточия:** `key:value` — неверно, нужно `key: value`.\n\n" +
						"## Как проверить файл\n\n" +
						"Редакторы подсвечивают YAML и показывают ошибку. Есть и отдельный линтер `yamllint`. " +
						"Проверяйте отступы перед запуском — это дешевле, чем ловить непонятную ошибку потом.\n\n" +
						"## Запомнить\n\n" +
						"1. YAML — язык конфигов: пары «ключ: значение», вложенность отступами, списки через дефис.\n" +
						"2. Только пробелы, никаких табов; обычно два пробела на уровень.\n" +
						"3. Один сбитый отступ ломает весь файл — всегда проверяйте.",
					"resources": []map[string]any{
						{
							"title": "YAML — официальный сайт",
							"url":   "https://yaml.org/",
							"note":  "спецификация формата и ссылки на инструменты",
						},
						{
							"title": "Learn YAML in Y minutes",
							"url":   "https://learnxinyminutes.com/docs/yaml/",
							"note":  "короткая шпаргалка по всему синтаксису на одной странице",
						},
					},
				},
			},
			{
				...
				Content: map[string]any{
					"body": "## Зачем это нужно\n\n" +
						...
						"```dockerfile\n" +
						"FROM golang:1.25-alpine     # берём образ Go; alpine — минимальный «облегчённый» Linux\n" +
						...
						"proj-db-1    postgres:17   Up 9 seconds                 5432/tcp\n" +
						"\n" +
						...
						"## ❓ Проверьте себя\n\n" +
						"Меняете одну строку кода — и `docker build` каждый раз заново качает все зависимости по несколько минут. Что не так в Dockerfile?\n\n" +
						"*Ответ: `COPY . .` стоит выше `RUN go mod download`. Любая правка кода рушит кэш слоёв ниже, поэтому зависимости качаются заново. Сначала скопируйте `go.mod` и `go.sum`, потом `RUN go mod download`, и только затем `COPY . .`. Тогда зависимости берутся из кэша.*\n\n" +
						"## Запомнить\n\n" +
				...
				Content: map[string]any{
					...
					"questions": []map[string]any{
						...
						},
						{
							"id":   "q8z",
							"type": "order",
							"text": "Расставьте путь образа от сборки до запуска на сервере:",
							"items": []map[string]any{
								{"id": "s1", "text": "Собрать образ: docker build"},
								{"id": "s2", "text": "Отправить в реестр: docker push"},
								{"id": "s3", "text": "Скачать на сервере: docker pull"},
								{"id": "s4", "text": "Запустить контейнер: docker run"},
							},
							"explanation": "Образ собирают, кладут в реестр, оттуда его забирает сервер и запускает как контейнер.",
						},
						{
							"id":          "q9z",
							"type":        "blank",
							"text":        "Впишите название файла с инструкциями по сборке образа.",
						...
						},
						{
							"id":   "m1",
							"type": "match",
							"text": "Сопоставьте команду Docker с её назначением:",
							"pairs": []map[string]any{
								{"id": "p1", "left": "docker build", "right": "собрать образ по Dockerfile"},
								{"id": "p2", "left": "docker run", "right": "запустить контейнер из образа"},
								{"id": "p3", "left": "docker logs", "right": "показать логи контейнера"},
								{"id": "p4", "left": "docker ps", "right": "список работающих контейнеров"},
							},
							"explanation": "Сборка, запуск, логи и список — ежедневные команды Docker.",
						},
					},
				},
			},
			{
				...
				Content: map[string]any{
					"body": "## Зачем это нужно\n\n" +
						...
						"## ❓ Проверьте себя\n\n" +
						"Сайт в контейнере жутко тормозит, а в `docker logs` ничего подозрительного. Какой командой в реальном времени увидеть, кто ест процессор и память?\n\n" +
						"*Ответ: `docker stats` — показывает CPU % и память по каждому контейнеру вживую. Логи фиксируют события, а не текущую нагрузку, поэтому тормоза в них не видны. Сразу заметно, если контейнер упёрся в лимит памяти или грузит процессор.*\n\n" +
						"## Запомнить\n\n" +
