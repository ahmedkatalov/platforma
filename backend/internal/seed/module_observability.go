		...
		Lessons: []LessonSeed{
			{
				...
				Content: map[string]any{
					"body": "## 🎯 Чему вы научитесь\n\n" +
						"- Различать метрики и логи и понимать, что смотреть при разборе поломки\n" +
						"- Следить за четырьмя золотыми сигналами и читать процентили p95/p99 вместо среднего\n" +
						"- Настроить алерт в Prometheus с порогом и выдержкой for и проверить правила promtool\n" +
						"- Считать SLI, SLO и бюджет ошибок и решать по нему — катить новое или чинить надёжность\n" +
						"- Найти причину инцидента по метрикам и логам: curl /metrics, grep ERROR, df -h, free -h\n" +
						"\n" +
						"## 📋 Что нужно знать заранее\n\n" +
						"- Работа в терминале: grep, wc, cat, journalctl, df -h, free -h (глава 2)\n" +
						"- Основы HTTP и запросы через curl (глава 4)\n" +
						"- Docker и просмотр логов контейнера через docker logs (глава 5)\n" +
						"- Чтение YAML-конфигов (глава 5)\n" +
						"- Базовые команды Kubernetes: kubectl logs и kubectl get pods (глава 8)\n" +
						"\n" +
						"## Зачем это нужно\n\n" +
						...
						"```\n\n" +
						"Здесь `sum(rate(5xx)) / sum(rate(всех))` — это **доля** ошибочных ответов среди всех. " +
						"Правило срабатывает, когда ошибок больше 5%. Просто `rate(5xx)` дал бы число в секунду, а не долю.\n\n" +
						...
						"\n" +
						"## ❓ Проверьте себя\n\n" +
						"Метрика показала всплеск ошибок. Где искать, ЧТО именно сломалось?\n\n" +
						"*Ответ: в логах. Метрика сигналит о проблеме, а лог объясняет её детали.*\n\n" +
						"## Запомнить\n\n" +
			...
			},
			{
				...
				Content: map[string]any{
					"body": "## Зачем это нужно\n\n" +
						...
						"\n" +
						"## ❓ Проверьте себя\n\n" +
						"Приложение шлёт данные, но графиков в Grafana нет. В логах коллектора видно `spans: 128`, а рядом `connection refused` на порт `4317`. Что происходит?\n\n" +
						"*Ответ: приём данных идёт, но коллектор не может отдать их дальше в бэкенд, поэтому спаны копятся в очереди. Проверьте адрес экспортёра и то, что хранилище живо: `connection refused` значит, что бэкенд не отвечает.*\n\n" +
						"## Запомнить\n\n" +
			...
			},
			{
				...
				Content: map[string]any{
					"body": "## Зачем это нужно\n\n" +
						...
						"## ❓ Проверьте себя\n\n" +
						"Замерили SLI и получили `0.9987` при цели SLO 99.9%. Команда просит выкатить новую фичу — разрешить?\n\n" +
						"*Ответ: нет: `0.9987` — это 99.87%, то есть цель 99.9% не выполнена и бюджет ошибок уже исчерпан. Пока не восстановится, катим не новое, а надёжность — иначе рискуем сделать пользователям ещё хуже.*\n\n" +
						"## Запомнить\n\n" +
				...
				Content: map[string]any{
					...
					"questions": []map[string]any{
						...
						},
						{
							"id":   "q8z",
							"type": "order",
							"text": "Расставьте порядок разбора инцидента:",
							"items": []map[string]any{
								{"id": "s1", "text": "Пришёл алерт"},
								{"id": "s2", "text": "По метрикам смотрим, ЧТО сломалось"},
								{"id": "s3", "text": "По логам выясняем, ПОЧЕМУ"},
								{"id": "s4", "text": "Исправляем"},
								{"id": "s5", "text": "Пишем постмортем без поиска виноватых"},
							},
							"explanation": "Метрика говорит ЧТО, лог — ПОЧЕМУ; постмортем меняет систему, а не наказывает человека.",
						},
						{
							"id":          "q9z",
							"type":        "blank",
							"text":        "Впишите одним словом: числовой показатель во времени — запросы, ошибки, память.",
							"accept":      []string{"метрика"},
							"hint":        "Противоположность текстовому логу.",
							"explanation": "Метрика — число во времени; лог — текстовая запись о конкретном событии.",
						},
						{
							"id":   "m1",
							"type": "match",
							"text": "Сопоставьте понятие мониторинга с определением:",
							"pairs": []map[string]any{
								{"id": "p1", "left": "Метрика", "right": "число во времени"},
								{"id": "p2", "left": "Лог", "right": "запись о событии"},
								{"id": "p3", "left": "Алерт", "right": "сигнал дежурному"},
								{"id": "p4", "left": "SLO", "right": "цель по надёжности"},
							},
							"explanation": "Метрика — что, лог — почему, алерт — когда будить, SLO — сколько надёжности обещаем.",
						},
					},
				},
			},
