		...
		Lessons: []LessonSeed{
			{
				...
				Content: map[string]any{
					"body": "## 🎯 Чему вы научитесь\n\n" +
						"- Объяснить путь запроса от домена до ответа: DNS, IP, порт, соединение\n" +
						"- Различать коды ответа HTTP и по цепочке находить виновника (500 против 502)\n" +
						"- Собрать обратный прокси nginx с TLS, X-Real-IP и редиректом HTTP на HTTPS\n" +
						"- Диагностировать неработающий сайт командами curl, dig, ss, journalctl, grep\n" +
						"- Проверять срок сертификата и безопасно хранить приватный ключ\n" +
						"\n" +
						"## 📋 Что нужно знать заранее\n\n" +
						"- Базовые команды терминала Linux и понимание путей к файлам\n" +
						"- Чтение и фильтрация текста: grep, wc, cat\n" +
						"- Права доступа к файлам и команда chmod\n" +
						"- Общее представление о роли DevOps-инженера и эксплуатации\n" +
						"\n" +
						"## Зачем это нужно\n\n" +
						...
						"## Что такое IP-адрес\n\n" +
						"IP-адрес — это **номер машины в сети**, например `93.184.216.34`. По нему находят сервер, " +
						"как дом по адресу на конверте.\n\n" +
						"Особый адрес `127.0.0.1` (его зовут `localhost`) всегда значит «этот же компьютер».\n\n" +
						"Адреса вида `10.x.x.x` и `192.168.x.x` — внутренние: видны только в своей сети, " +
						"из интернета к ним напрямую не достучаться.\n\n" +
						"## Что такое порт\n\n" +
						...
						"\n" +
						"## ❓ Проверьте себя\n\n" +
						"Один сервис вернул 500, другой — 502. В чём разница между этими ошибками?\n\n" +
						"*Ответ: 500 — программа ответила ошибкой (сломалось внутри неё); 502 — прокси не дозвонился до приложения.*\n\n" +
						"## Запомнить\n\n" +
			...
			},
			{
				...
				Content: map[string]any{
					"body": "## Зачем это нужно\n\n" +
						...
						"## ❓ Проверьте себя\n\n" +
						"Сайт открывается, но браузер пишет «Не совпадает имя». Сертификат выписан на `www.example.com`, а домен — `app.example.com`. В чём дело?\n\n" +
						"*Ответ: сертификат привязан к конкретному имени, а этот выдан для другого домена. Нужно перевыпустить его на `app.example.com`, тогда браузер перестанет ругаться.*\n\n" +
						"## Запомнить\n\n" +
				...
				Content: map[string]any{
					...
					"questions": []map[string]any{
						...
						},
						{
							"id":   "q7z",
							"type": "order",
							"text": "Расставьте по порядку, что происходит, когда вы открываете сайт:",
							"items": []map[string]any{
								{"id": "s1", "text": "Браузер узнаёт IP по домену (DNS)"},
								{"id": "s2", "text": "Устанавливается TCP-соединение"},
								{"id": "s3", "text": "Согласуется шифрование (TLS/HTTPS)"},
								{"id": "s4", "text": "Браузер отправляет HTTP-запрос"},
								{"id": "s5", "text": "Сервер возвращает ответ"},
							},
							"explanation": "Сначала находим адрес и поднимаем защищённое соединение, и только потом идёт обмен запросом и ответом.",
						},
						{
							"id":          "q8z",
							"type":        "blank",
							"text":        "Впишите команду с флагом, которая проверяет конфигурацию nginx, ничего не применяя: nginx …",
							"accept":      []string{"-t", "nginx -t"},
							"hint":        "Один флаг — «test».",
							"explanation": "nginx -t ловит опечатку в конфиге до перезагрузки и спасает сайт от падения.",
						},
						{
							"id":   "m1",
							"type": "match",
							"text": "Сопоставьте код ответа HTTP с его смыслом:",
							"pairs": []map[string]any{
								{"id": "p1", "left": "200", "right": "всё хорошо"},
								{"id": "p2", "left": "404", "right": "такой страницы нет"},
								{"id": "p3", "left": "500", "right": "ошибка внутри приложения"},
								{"id": "p4", "left": "502", "right": "прокси не дозвонился до приложения"},
							},
							"explanation": "Код ответа сразу подсказывает, на чьей стороне искать проблему.",
						},
					},
				},
			},
