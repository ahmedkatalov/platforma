		...
		Lessons: []LessonSeed{
			{
				...
				Content: map[string]any{
					"body": "## 🎯 Чему вы научитесь\n\n" +
						"- Настроить хранение секретов вне кода: .env, .gitignore и хранилища CI, Kubernetes, Vault\n" +
						"- Отозвать утёкший секрет и объяснить, почему удаление коммита его не спасает\n" +
						"- Выставить права 600 на приватный ssh-ключ и применить принцип наименьших прав\n" +
						"- Собрать безопасный Dockerfile: фикс версии, не-root, многоэтапная сборка, EXPOSE\n" +
						"- Найти уязвимости образа сканером Trivy и проверить открытые порты через ss -tulpn\n" +
						"\n" +
						"## 📋 Что нужно знать заранее\n\n" +
						"- Linux и терминал: права на файлы (chmod), grep и базовые команды\n" +
						"- Git: коммиты, история и файл .gitignore\n" +
						"- Сети и HTTP: порты и что открыто наружу\n" +
						"- Docker: сборка образов и структура Dockerfile\n" +
						"- CI/CD: конвейер сборки и его секреты\n" +
						"\n" +
						"## Зачем это нужно\n\n" +
						...
						"> **Важно про Kubernetes Secret.** По умолчанию это не шифрование, а лишь `base64` — " +
						"его легко раскодировать. Для настоящей защиты включают шифрование в etcd, ограничивают " +
						"доступ через RBAC и берут внешние менеджеры: Vault, External Secrets, Sealed Secrets или SOPS.\n\n" +
						...
						"\n" +
						"## ❓ Проверьте себя\n\n" +
						"Токен закоммитили и через минуту удалили следующим коммитом. Он в безопасности?\n\n" +
						"*Ответ: нет. `git show` достаёт его из старого коммита. Секрет в Git считается скомпрометированным — его отзывают и выпускают заново.*\n\n" +
						"## Запомнить\n\n" +
			...
			},
			{
				...
				Content: map[string]any{
					"body": "## Зачем это нужно\n\n" +
						...
						"<missing>      3 hours ago   FROM alpine:3.22                 7MB\n" +
						"\n" +
						...
						"## ❓ Проверьте себя\n\n" +
						"В Dockerfile прописали пароль через `ENV`, потом файл с ним удалили. Коллега говорит, что образ всё равно небезопасен. Прав ли он?\n\n" +
						"*Ответ: Да, прав: пароль остался в слоях образа. Его видно командой `docker history`. Удаление файла слой не стирает — образ надо пересобрать заново, без секрета в сборке.*\n\n" +
						"## Запомнить\n\n" +
				...
				Content: map[string]any{
					...
					"questions": []map[string]any{
						...
						},
						{
							"id":   "q7z",
							"type": "order",
							"text": "Ключ доступа случайно попал в git. Расставьте действия по порядку:",
							"items": []map[string]any{
								{"id": "s1", "text": "Немедленно отозвать и заменить ключ"},
								{"id": "s2", "text": "Убрать секрет из кода"},
								{"id": "s3", "text": "Почистить историю репозитория"},
								{"id": "s4", "text": "Хранить секрет в Vault или секретах CI"},
							},
							"explanation": "Что попало в git, считается скомпрометированным, поэтому сначала отзыв ключа, а потом уборка.",
						},
						{
							"id":          "q8z",
							"type":        "blank",
							"text":        "Принцип «выдавать минимально необходимые права» — это принцип наименьших …",
							"accept":      []string{"привилегий", "привилегии"},
							"hint":        "Родственно слову «привилегия».",
							"explanation": "Меньше прав — меньше ущерб при взломе или ошибке.",
						},
					},
				},
			},
